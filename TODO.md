...
- [ ] **Фаза 7** — Конфигурация и безопасность · FE 5/5 · BE 0/2 · *env, токены, осмысленный вход*
...
- [ ] **7.4** [FE+BE] (P2) Авторизация/идентификация
      ...
      BE-часть: код входа/токена готов с 3.10, критерий закрывается тестом (см. BE-коммит).
...
Снимок фактов на 12.09.2026 (после Фаз 0–2, всей Фазы 3, Фазы 4: 4.1–4.5, Фазы 5: 5.1–5.6 и
Фазы 6: 6.1–6.3; по Фазе 4 остаётся живой прогон на Android — в окружении нет JDK/Android SDK).
Проверенные факты:
...
+ BE `6b34f94` (5.6), Фаза 6 — FE `aaaa50c` (6.1–6.3) (см. `git log`).
**Фазы 3, 4 и 5 закрыты, Фаза 6 закрыта** — vitest (**79 тестов**: миграции, репозитории, синк,
автосинк и индикатор) + PHPUnit в бэкенде (`php artisan test` → `30 passed`, exit 0), из рантайм-проверок
...
Дальше — Фаза 7 (конфигурация и безопасность: env, токены, осмысленный вход);
⚠️ синк требует токен, поэтому **7.4 (вход/получение токена на клиенте) остаётся блокирующей для
синхронизации** — до неё устройства получают 401 (данные не теряются: очередь растёт, а индикатор
показывает количество несинхронизированных операций).
