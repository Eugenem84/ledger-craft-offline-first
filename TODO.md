...
- Уже сделано: `$tables` (0.8), agent log (0.9), идемпотентность + SAVEPOINT/вырезание `*_server_id` (3.5,
  ...
  `ApiSmokeTest` (5.6), вход/токен (`tests/Feature/AuthSyncTest.php` — 5 тестов, 7.4) и гигиена API
  (7.6). `php artisan test` → `35 passed`, exit 0 (2 теста помечены «deprecated» — шум PHP 8.5 +
  vendor, не падения). Продуктовый вопрос по web-версии решён — **D3: web-часть признана продуктом**,
  поэтому удаляли только мёртвое/дублирующее.
