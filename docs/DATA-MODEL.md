...
- ⚠️ **Исправлено 15.09.2026 (жалоба «в статистике прочерки и нули, хотя товар заведён»).** Формулы
  были верны, терялись данные: `useOrderDraftStore.updateOrder()` пересоздаёт строки заказа
  («удалить и добавить заново») и **не передавал `buy_price`** — себестоимость стиралась при каждом
  сохранении правки, и «Аналитика» показывала закупку 0, маржу на всю выручку и наценку `—`.
  Сохранять `buy_price` теперь обязательно и в `updateOrder()` (регрессия — `test/order-margin.test.js`).
  Плюс цену закупки можно задать **в карточке товара** (`useProductsStore.saveBuyPrice()` →
  `buyProductPricesRepo.saveBuyPrice()`, та же семантика «одна актуальная закупка на товар»), а не
  только приходом; наценка-прочерк «Аналитика» объясняет текстом под итогами.
...
| таблица | поле | UI |
|---|---|---|
| `order_service` | `quantity` (работы) | шагомер «‹ N ›» в каталоге работ — только у уже выбранной работы и только в режиме правки; колонка «кол-во» в списке работ заказа |
| `order_product` | `quantity` (товары со склада) | шагомер «‹ N ›» в диалоге товара (добавить сразу N) + редактор строк |
| `materials` | `amount` (ручные позиции) | шагомер «‹ N ›» в диалоге ручной позиции + редактор строк |

Количество задаётся **явными стрелками влево/вправо** (`LcQuantityStepper`, правка владельца
15.09.2026): системные стрелки «вверх/вниз» у поля с `type="number"` на телефоне не попадали
по пальцу. Нормализация — в `src/utils/quantity.js` (`−3` → 1, `2.5` → 2, пусто → 1) на всех
границах: поле ввода, стор и репозитории перед записью в БД и в payload синка.

⚠️ **Остаток склада не путается с количеством строки.** У товара со склада в объекте есть
`quantity` — это остаток (`product_stocks`), а количество строки заказа — `amount`. Раньше
остаток уезжал в черновик заказа, и итоги с себестоимостью считались по остатку, а не по
введённому количеству (дефект разобран 15.09.2026): `useOrderDraftStore.addProductFromStore()`
остаток не переносит, а `lineQuantity()` берёт `amount` первым.
...
### История склада (правка владельца 15.09.2026)

«В разделе склад нужна история приходов и расходов товаров», «две вкладки: товары и …
история движения по складу … и редактирование» — отдельной таблицы для истории не заводим:
движения уже лежат в двух местах, и `stockHistoryRepo` сводит их в одну ленту.

| движение | источник | цена | знак в UI |
|---|---|---|---|
| приход | `incoming_products` (рубрика «Поступление», 9.2) | `by_price` (закупка) | `+` |
| расход | `order_product` JOIN `orders`, `deleted_at IS NULL` с обеих сторон | `sale_price` | `−` |

Строка ленты — `{ id, arrivalId, productId, productName, kind, quantity, price, supplier,
synced, createdAt, title }` (`title` — поставщик или `ордер №N`), новые движения сверху;
`useStockHistoryStore` отдаёт ещё и итоги «пришло / ушло». Два режима чтения:

- `getByProductId()` — история одного товара: карточка товара на складе
  (`ProductDialogPage.vue`), без лимита;
- `getAll(specializationId)` — движение **всех** товаров профиля: вкладка «движение товаров»
  на складе (`StoreHistoryPanel.vue`), лимит `HISTORY_LIMIT = 200` (в UI об этом сказано).
  Профиль определяется категорией товара (`product_categories.specialization_id`) — та же
  привязка, по которой склад строит список товаров, поэтому чужие движения не подмешиваются.

> ⚠️ **Фильтр профиля — по двум формам ключа** (`specialization_id = ? OR specialization_id = ?`),
> пару отдаёт `specializationsRepo.resolveScopeKeys()`. В `product_categories.specialization_id`
> лежит либо локальный UUID (категория создана на устройстве), либо **серверный id** (категория
> приехала синком — `applyServerRecord` пишет `record.specialization_id` «как есть», парную
> `specialization_server_id` никто не заполняет). Одна форма делала вкладку пустой **только на
> Android** (там категории синхронизированы), пока в браузере лента показывалась — дефект
> 15.09.2026. Пустое состояние ленты дополнительно показывает счётчики движений по всей базе
> (`stockHistoryRepo.countAll()`), чтобы «не тот фильтр» было видно на экране.

Синка и миграций не потребовалось: репозиторий только читает, а удаление строки заказа убирает
расход из истории само (строка либо удаляется, либо помечается `deleted_at`).

#### Правка прихода (`incomingProductsRepo.updateArrival`)

Что можно менять — определяет сервер, а не UI: `IncomingProductRepository::recordArrival()`
увеличивает склад по приходу **ровно один раз** (идемпотентность по `uuid_id`), а при повторной
отправке/`update` только правит строку и **остаток не пересчитывает**.

| состояние прихода | количество | закупка, поставщик |
|---|---|---|
| ещё в очереди (`server_id IS NULL`) | правится: локальный остаток меняется **на дельту** (`productStocksRepo.adjustQuantity`), ожидающий INSERT в очереди переписывается (payload сериализуется в момент постановки) — на сервер уйдёт новое количество | правится (INSERT переписывается) |
| уже на сервере (`server_id` есть) | **заблокировано** — правка разошлась бы со складом; UI гасит шагомер и объясняет причину | правится обычной `update`-операцией (остаток эти поля не меняют) |

Правка цены закупки дополнительно уезжает в `buy_product_prices` (`saveBuyPrice`) — одна
актуальная закупка на товар, из неё считается маржа «Аналитики». Остаток никогда не уходит ниже
нуля (отрицательное количество — ошибка, а не склад). Регрессии: `test/stock-history.test.js`
(дельта, переписанный INSERT, запрет количества у синхронизированного прихода),
`test/store-movements-ui.test.js` (вкладки, кнопка «изменить», окно диалога).
