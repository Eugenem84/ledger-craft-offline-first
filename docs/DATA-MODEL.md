...
- ⚠️ **В карточке заказа себестоимость не показывается** (правка владельца 15.09.2026, задача 14.21):
  из итогов и строк убраны «закупка / маржа / наценка», из диалога ручной позиции — поле «Закупка».
  Данные при этом живут: `buy_price` синкается, маржа с наценкой считаются в «Аналитике»
  (и на сервере) по-прежнему.

## Сумма заказа: позиции, а не снапшот (задачи 9.1 и 14.20)

Сумма заказа в UI считается **только из позиций**: работы и товары `quantity × sale_price`,
ручные позиции `amount × price`, `deleted_at IS NULL`. `orders.total_amount` — снапшот,
он пишется при сохранении заказа из карточки и намеренно **не используется** в расчётах:

- аналитика — `src/database/queries/analytics.js` и серверный `StatisticRepository`
  (`billedOrdersSubquery()`), см. задачу 9.1;
- карточка заказа — геттеры `useOrderDraftStore` (`servicesTotal`/`materialsTotal`/`productsTotal`);
- список заказов — `positions_total` в `src/database/queries/orders.js` (задача 14.20);
  снапшот остался лишь фолбэком для заказов без позиций.

Причина: снапшот расходится с карточкой, если строки менялись в обход сохранения заказа
(приехали с сервера/второго устройства, «Починка очереди», восстановление бэкапа) или заказ
пришёл с сервера с пустым `total_amount`.

## Количество в позициях заказа (задача 14.19)

Количество — **целое ≥ 1** и всегда поле строки, а не дубли строк: у `order_service` на сервере
нет собственного PK, идемпотентность и удаление идут по натуральному ключу
`order_id + service_id`, поэтому «подкачать колесо ×4» — это одна строка с `quantity = 4`.

| таблица | поле | UI |
|---|---|---|
| `order_service` | `quantity` (работы) | поле «кол-во» в каталоге работ — только у уже выбранной работы и только в режиме правки; колонка «кол-во» в списке работ заказа |
| `order_product` | `quantity` (товары со склада) | поле «Количество» в диалоге товара (добавить сразу N) + редактор строк |
| `materials` | `amount` (ручные позиции) | поле «Количество» в диалоге ручной позиции + редактор строк |

Нормализация — в `src/utils/quantity.js` (`−3` → 1, `2.5` → 2, пусто → 1) на всех границах:
поле ввода (`min="1" step="1" inputmode="numeric"`), стор и репозитории перед записью
в БД и в payload синка.
