...
- индикатор `src/components/SyncStatusBar.vue` живёт **только** в шапке `MainLayout.vue`
  ...
  `src/utils/syncStatusView.js`. С правки владельца 15.09.2026 чип — **только значок** (круглый,
  24px, `lc-sync-chip--icon`) без подписи: состояние несут цвет и иконка, а рядом в шапке мелко и
  серым показана версия сборки (`MainLayout` + `useUpdateStore.currentVersionShort`, правила —
  `src/utils/appVersion.js`). Версия для веб-сборки вшита в бандл из `gradle.properties`
  (`build.env` в `quasar.config.js`), потому что нативного источника в браузере нет; у состояния
  «синхронизировано» свой насыщенный зелёный (`--lc-sync-ok`).
...
стал модальным окном). Оба пишут в `useSpecializationsStore.selectedId` (в окне настроек —
сразу при выборе, кнопок сохранения там нет).
...
- **Видимость вкладок** (10.3; доработка) — флаги профиля (`features`): состав вкладок
  ...
  настроек (`components/settings/SettingsDialog.vue`, экшен стора `setFeatures` — пишется сразу)
  пишут тот же JSON, что при создании профиля,
...
- **Режим разработчика** (12.4/12.5; доработка) — `components/dev/DeveloperPanel.vue` подключается
  ...
  во вкладке «разработка» окна настроек (`src/utils/devMode.js`; флаг пишется сразу, лежит в
  `localStorage` и переживает перезапуск).
...
- **Переключатели статуса/оплаты оформлены как чипы статуса** (15.09.2026). Активный
  сегмент заливался жёлтым (`toggle-color="secondary"`) и не отражал статус. Теперь он
  выглядит ровно как чип статуса из списка заказов (`.lc-status--*`): светофорный цвет
  (`--lc-waiting`/`--lc-process`/`--lc-done`) плюс мягкая заливка (12 %) и тонкая рамка (35 %)
  из тех же парных токенов `--lc-*-soft` / `--lc-*-line` (объявлены один раз в `app.scss`),
  подпись и иконка красятся цветом статуса; иконки сегментов — из общей карты
  `ORDER_STATUS_ICONS` (`src/utils/analytics.js`), кегль 14px, как у чипа (на экранах уже 360px
  иконки скрываются: ряд из четырёх сегментов иначе не помещался бы в карточку). Заливка —
  «переезжающий» индикатор: псевдоэлемент `.lc-toggle::before` (форма «пилюля») лежит **под**
  подписями (сегменты прозрачные) и анимируется через `transform` + `background-color`/
  `box-shadow` (0.28s), вместо мгновенной смены; рамку рисует `inset`-тень, потому что у
  псевдоэлемента нет собственных границ. Позиция и цвета — CSS-переменные классов
  `.lc-toggle--waiting/process/done` (`--lc-toggle-shift` = индекс статуса в `ORDER_STATUSES`;
  незнакомый статус — нейтральный серый индикатор). «Оплачено» — тот же вид из одного сегмента,
  индикатор появляется только у выставленного значения. ⚠️ Quasar красит активный сегмент
  классами палитры с `!important` (у `toggle-color` дефолт — `primary`), поэтому фон и цвет
  сегмента переопределены с `!important`. Регрессы — `test/phase12-profile.test.js`.
- **Кнопки, список заказов и тумблеры — тот же «чип»-язык** (15.09.2026, продолжение правок
  владельца). Цветные кнопки больше не сплошные плашки: `.q-btn.bg-secondary` / `bg-positive` /
  `bg-negative` / `bg-deep-orange` / `bg-warning` в `app.scss` получают мягкую заливку 12 %,
  кольцо 35 % (inset-тень — геометрия и тап-зона не меняются) и насыщенную подпись со значком;
  у акцента мягкое свечение как метка главного действия (у FAB сильнее, плюс тень), форма —
  «пилюля». Так закрыты «новый заказ», «новая работа», «добавить материал» и кнопки обновления
  («доступна версия N», «Обновить», «Перезапустить сейчас»). Исключение — индикатор синка в
  шапке (`.lc-sync-chip--icon`): знак состояния, а не кнопка, он остаётся сплошным. Палитра
  сведена к четырём ролям + серым (`--lc-danger` / `--lc-ok` / `--lc-warn` = цвета статусов,
  `deep-orange` убран), подписи кнопок — строчными (`text-transform: none`), фокус в полях —
  жёлтый акцент. Список заказов: `q-list.lc-order-list` + `.lc-order-row` — заказ стал отдельной
  карточкой (рамка, радиус, `gap: 10px`, полоса статуса-«пилюля»), раньше строки сливались в
  одно полотно. Тумблеры склада и «Аналитики» — класс `.lc-seg` (та же «таблетка» с мягким
  активным сегментом, палитровые пропсы из разметки убраны; в «Аналитике» строка переносится —
  тумблер не обрезается на 360px). Индикатор синка: «нет сети» — янтарный (`syncStatusView`),
  чтобы не сливаться с красной «ошибкой». Регрессы — `test/ui-buttons.test.js`.
...
## 12. OTA веб-слоя: обновление без установки APK (Фаза 15) — реализовано на клиенте

Задачи **15.1–15.6, 15.8, 15.10** сделаны; остались бэкенд-команда публикации (**15.7**) и живой
прогон (**15.9**). Решение — **D8**, контракт — `docs/API-INTEGRATION.md` §2.5, пользовательское
описание — README §«Обновление без установки: OTA веб-слоя».

Идея: манифест релиза (`GET /api/app-version`) может содержать объект **`bundle`** — zip с
веб-сборкой. Если он есть, обновление ставится **без установки APK**: плагин
`@capawesome/capacitor-live-update` скачивает архив, сверяет `checksum` (sha256 в base64) и
применяет бандл при следующем запуске. Нативный путь (APK + `PackageInstaller`, Фаза 13) остаётся
для нативных изменений — и он **приоритетнее**: новый APK приносит и новый веб-слой.

| Файл | Роль |
|---|---|
| `src/utils/liveUpdate.js` | мост к плагину (динамический импорт, как у `ApkInstaller`): `ready`, `getCurrentBundleId`, `hasLiveUpdatePlugin`, `downloadBundle` (с прогрессом), `setNextBundle`, `reloadApplication`, `reset`; у коротких вызовов — таймаут 3 с (`NATIVE_CALL_TIMEOUT_MS`), чтобы молчащий плагин не держал вызывающего; `getAppVersionCode()/getAppVersionName()` — второй, независимый источник своей версии |
...
- **Авто-откат** делает плагин: `readyTimeout: 10000` + `autoBlockRolledBackBundles: true`
  (`src-capacitor/capacitor.config.json`). Если бандл не позвал `ready()` за 10 с, плагин вернётся
  к встроенному бандлу и заблокирует плохой.
- **Миграции БД в OTA-релизах — только аддитивные.** Откат на предыдущий бандл должен уметь
  открыть базу, созданную новым; переименования/удаления колонок выкатываются вместе с APK.
- **`minNativeVersionCode`** в манифесте: бандл не предлагается на старом APK (страховка от «новый
  JS зовёт новый плагин»).
- **После установки нового APK** плагин сам возвращается к встроенному бандлу — значит веб-часть
  внутри APK должна быть свежей.
- Режим веб-плагина — **ручной** (`autoUpdateStrategy: 'none'`): решение принимает
  `updateService`, а не плагин; никакого облака, только свой сервер.
- **Своя версия — из двух источников.** `App.getInfo()` (`@capacitor/app`), а если он не ответил
  или ответил пусто — `getVersionCode()/getVersionName()` плагина OTA (другой Java-путь к тому же
  `PackageInfo`), затем кэш в хранилище. У обоих таймаут 3 с (`src/utils/async.js`), провал
  **не запоминается** — это и был дефект «версия приложения неизвестна» на Android (сборка 1.10,
  исправлено в 1.11).
- **Прокси плагина нельзя «ожидать».** Capacitor отдаёт плагин прокси-объектом: на незнакомое
  свойство он отвечает ошибкой «не реализовано», а `then` — как раз незнакомое. Промис, который
  «оседает» на таком прокси, **не завершается никогда** (движок вызывает `then` как thenable, а
  прокси оставляет промис в ожидании) — так «повисала» вся проверка обновлений в 1.11–1.12
  (отчёт мастера: `"LiveUpdate.then()" is not implemented on android`). Наружу отдаём только
  обычную обёртку: `loadLiveUpdate()` → объект с привязанными методами, `loadApkInstaller()` →
  `{ ApkInstaller }`. Держит `test/live-update-bridge.test.js` (плагин подменён «строгим» прокси).
- **Любой `await` на плагине — под таймаутом.** Включая динамический импорт чанка плагина
  (`src/utils/async.js` → `withTimeout`): иначе не загрузившийся чанк держит вызывающего.
- **`status` в сторе — только состояние сервиса.** `refreshCurrentVersion()` возвращает **версию**,
  а не состояние: присваивать её в `this.status` нельзя (дефект 1.11 — карточка показывала
  «обновления ещё не проверялись», кнопка выглядела мёртвой). Держит `test/update-store.test.js`.

- **`minNativeVersionCode` — код сборки, для которой бандл собран.** Клиент не предлагает бандл ни
  когда он требует более нового APK (`native < min`), ни когда он собран для **более старого**
  (`0 < min < native`): во втором случае встроенный веб-слой APK не старее, и «обновление без
  установки» откатило бы мастеру интерфейс (задача 15.16; `0`/пусто = «ограничения нет»).
- **После установки нового APK** Capacitor сбрасывает веб-слой на встроенный
  (`Bridge.isNewBinary()` → `CAP_SERVER_PATH = ""`), поэтому «применится при перезапуске» снимается
  (`_dropStalePendingBundle`, задача 15.17), а не висит обещанием в никуда.
- **Нативный релиз выкатывается вместе с бандлом**: `npm run release:android -- --with-bundle` (или два
  шага вручную) — иначе на контуре остаётся бандл, собранный для прошлой сборки. Детали — README
  §«Нативное обновление: что происходит с OTA-бандлом».
