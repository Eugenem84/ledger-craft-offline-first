...
- **Переключатели статуса/оплаты: цвет по статусу и «переезд» индикатора** (15.09.2026).
  Активный сегмент заливался жёлтым (`toggle-color="secondary"`) и не отражал статус. Теперь
  цвета светофорные, как у чипов (`--lc-waiting`/`--lc-process`/`--lc-done`), а заливка —
  «переезжающий» индикатор: псевдоэлемент `.lc-toggle::before` лежит **под** подписями
  (сегменты прозрачные) и анимируется через `transform` + `background-color` (0.28s), вместо
  мгновенной смены. Позиция и цвет — CSS-переменные классов `.lc-toggle--waiting/process/done`
  (`--lc-toggle-shift` = индекс статуса в `ORDER_STATUSES`; незнакомый статус — нейтральный
  серый индикатор). «Оплачено» — тот же вид из одного сегмента, индикатор появляется только у
  выставленного значения. ⚠️ Quasar красит активный сегмент классами палитры с `!important`
  (у `toggle-color` дефолт — `primary`), поэтому фон и цвет сегмента переопределены с
  `!important`. Регрессы — `test/phase12-profile.test.js`.
