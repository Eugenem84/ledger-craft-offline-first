...
3. **Деньги**: `orders.total_amount` клиент хранит в **копейках** и конвертирует `/100`,
   остальные цены — в рублях. На сервере должно быть единое правило (предположительно —
   рубли).
