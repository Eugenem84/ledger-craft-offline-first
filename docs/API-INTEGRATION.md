...
```json
{
  ...
  "apk_name": "2-1.1.apk",
  "bundle": {
    "version": "1.8.260915-1108",
    "url": "https://dev.medovf2h.beget.tech/api/download-bundle?version=1.8.260915-1108",
    "checksum": "i7bZ0k…=",
    "sizeBytes": 812345,
    "minNativeVersionCode": 10,
    "notes": "Правки склада"
  }
}
```
...
- **`bundle` (OTA веб-слоя, Фаза 15)** — необязательный объект:
  - `version` — идентификатор бандла (**строка**, не версия для сравнения: клиент считает бандл
    своим при совпадении строки, «какой новее» решает сервер, отдавая актуальный);
