...
Клиент с задачи **14.19** шлёт `quantity` всегда (целое ≥ 1): «подкачать колесо ×4» — одна строка
с количеством 4 (без поля сервер проставил бы 1).
...
✅ Ручка закрыта (задача 11.7, бывший O-6): `auth:sanctum` + проверка владельца товара — без
токена/сессии `401`, чужой товар → `403 FORBIDDEN_NOT_OWNER`. Web-версия проходит по сессии
(`withCredentials` + `X-CSRF-TOKEN`, first-party Sanctum), bearer-токен тоже принимается.
...
## 2.5. `GET /api/app-version` — версия приложения, раздача APK и OTA-бандла

Задача 13.1 (+ OTA веб-слоя, Фаза 15). Эндпоинт обслуживает обновление Android-клиента:
приложение сравнивает свой нативный `versionCode` с ответом и предлагает скачать и установить
новую сборку. Если в ответе есть `bundle` (веб-слой) — обновление ставится **без установки**:
клиент скачивает zip, сверяет хэш и применяет его при следующем запуске.
...
```json
{
  ...
  "apk_name": "2-1.1.apk",
  "bundle": {
    "version": "1.14.7.260915-1440",
    "url": "https://dev.medovf2h.beget.tech/api/download-bundle?version=1.14.7.260915-1440",
    "checksum": "843586bc9e3ca550fab7a74826094f062ba74ff158d03759918735c1e6a51bcf",
    "checksumBase64": "hDWGvJ48pVD6t6dIJglPBiunT/FY0DdZkYc1wealG88=",
    "sizeBytes": 1167859,
...
- **`versionCode` — критерий «новее» для APK** (Android сравнивает именно его; строку `1.10` так
...
- **`bundle` (OTA веб-слоя, Фаза 15)** — необязательный объект:
  - `version` — идентификатор бандла (**строка**, не версия для сравнения: клиент считает бандл
    своим при совпадении строки, «какой новее» решает сервер, отдавая актуальный). Формат —
    `<версия APK>.<номер веб-релиза>.<ддммгг-ччмм>`, напр. `1.14.7.260915-1440` (задача 15.21):
    префикс — APK-линия (растёт только нативным релизом), номер — какой это по счёту выкат
    веб-слоя, время — уникальность. Идентификатор собирает `release-web.sh`, следующий номер он
    берёт от манифеста контура (`APP_BUNDLE_BUILD` в `gradle.properties` — только пол, если
    манифест недоступен), причём только у бандла **той же APK-линии**: после подъёма версии APK
    нумерация начинается заново. Серверу формат безразличен — он хранит строку как есть;
  - `url` → `GET /api/download-bundle?version=<id>` (zip с веб-сборкой, файлы в корне архива);
  - `checksum` — sha256 файла **в hex** (⚠️ именно hex: плагин OTA на устройстве считает sha256
    скачанного zip и приводит его к hex, а base64 отвергает ошибкой «Checksum mismatch» —
    проверено живым прогоном 15.09.2026, задача 15.15);
  - `checksumBase64` — тот же хэш в base64, только для справки (клиент его не использует);
  - `sizeBytes` — размер zip (клиент показывает его в диалоге);
  - `minNativeVersionCode` — **код сборки, для которой бандл собран** (не «минимум»): клиент не
    предложит бандл на более старом APK (страховка «старый APK + новый JS») и, наоборот, не предложит
    бандл, собранный для **более старого** APK, — иначе новый APK откатил бы себе веб-слой
    (задача 15.16). `0`/пусто — «ограничения нет»;
  - `notes` — «что нового» для этого бандла (если пусто — берутся `notes` релиза).
  Блок `bundle` можно отдавать и **без** новых `versionCode`: ответ тогда означает «обнови веб-слой,
  APK ставить не нужно».
...
- Публикация APK — `php artisan app:publish-apk` (см. `LedgerCraftDocker03/docs/ENVIRONMENTS.md`).
  Публикация бандла — `php artisan app:publish-bundle <zip> --bundle-version=<id> --checksum=<base64>
  --min-native-version=<code> [--notes=…]` (Фаза 15, задача 15.7 — реализована; ⚠️ имя опции
  именно `--bundle-version`, потому что `--version` у Symfony Console глобальный и «съедает»
  команду: публикация молча ничего не делала).

Со стороны клиента: `src/services/updateService.js` (проверка, скачивание, установка APK и OTA),
`src/utils/appUpdateView.js` (чистая логика «есть обновление / обязательно / отложено / бандл»),
`src/utils/liveUpdate.js` (мост к плагину OTA), чип «доступна версия N» / «обновление без установки»
— `src/components/UpdateBanner.vue`, диалог — `UpdateDialog.vue`, вкладка «обновление» в окне
настроек (`src/components/settings/SettingsDialog.vue`).

### 2.6. `POST /api/feedback` — отчёты об ошибках (Фаза 14, реализовано)

Кнопка «Сообщить об ошибке» — в настройках, вкладка «поддержка» (`settings/SettingsDialog.vue`).
Отчёт — **не** таблица синка (решение **D7**): у него свой контур, потому что «полный сброс»/бэкап
не должны его тащить, а второе устройство владельца не
