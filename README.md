...
3. Синхронизация связных таблиц `order_product` / ручных позиций материалов не доработана
   (их нет в `syncService`) — задача 3.4; по решению D2 новая таблица `order_material` не нужна,
   а клиентский справочник `materials` удаляется. Двойной вызов `_syncLocalToServer` из `sync()`
   убран (задача 3.1), порядок «родитель → ребёнок» и «дожим» отложенных операций делает
   топологическая сортировка + волны внутри одного `sync()` (задача 3.2).
