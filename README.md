...
| **prod-VPS** | `https://<prod-домен>/api` (домен уточняется — TODO 11.1) | боевой контур мастерских | реальные данные: только бэкап + проверенное обновление |

Правило простое: **фича сначала обкатывается на dev**, проверяется по чек-листу (TODO 11.5) и только
потом уходит в бой (TODO 11.12). Код один и тот же — сборки различаются адресом API.

```bash
# dev-сборка (адрес берётся из отслеживаемого .env)
npm run build                      # → dist/spa

# prod-сборка: положить рядом .env.local (он в .gitignore) с боевым адресом
#   VITE_API_URL=https://<prod-домен>/api
npm run build                      # → dist/spa
```

`dist/spa` — статика с hash-роутером (`index.html`, `assets/`, `sql-wasm.wasm`); выкладывается на
хост нужного контура. Адрес API нигде не зашит в код: он приходит из env (задача 7.1), поэтому
смена контура — это правка `.env`/`.env.local`, а не релиз кода.
Серверная часть (Laravel, миграции, домен Traefik) описана в `LedgerCraftDocker03/README.md`.
