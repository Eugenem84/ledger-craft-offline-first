...
профиля, акцентный цвет — из пресета, а лишние вкладки (склад, аналитика) скрываются флагами.
Подробности — `docs/PLAN.md` §Фаза 10 и `docs/FRONTEND.md` §9.
...
только переключить активный или добавить ещё одну нишу. Отладочные инструменты (снимок окружения и
синка, очередь операций, буфер логов, полный сброс и удаление локальной БД) вынесены в секцию
«Режим разработчика», которая есть только в dev-сборке. Подробности — `docs/FRONTEND.md` §10,
`docs/UI.md` §5–6.
