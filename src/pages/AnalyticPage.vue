  ...
  <q-page class="lc-page lc-shell">
    ...
    <template v-else>
      ...
      <div class="lc-card lc-pad q-mb-md">
        ...
        <!-- Почему «закупка 0», а наценка — прочерк: считать её не из чего, пока у позиций
             нет цены закупки (товар заведён без «Поступления» и без правки закупки).
             Правка владельца 15.09.2026: прочерк должен быть объяснён, а не молчать. -->
        <div
          v-if="analytics.summary.revenue > 0 && analytics.summary.cost === 0"
          class="text-caption lc-mute q-mt-sm"
        >
          закупка не заполнена: укажите «Цену закупки» в карточке товара на складе или
          оформите «Поступление» — себестоимость позиций берётся оттуда
        </div>
      </div>
